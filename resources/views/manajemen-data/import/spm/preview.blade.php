...
@section('title', 'Preview Import SPM')

@php($labelJenis = $import->jenis_spm === 'ls' ? 'LS' : 'UP/GU')

@section('content')
<div class="dash-card">
    <h3>Preview Import SPM {{ $labelJenis }}</h3>
    ...
    @if ($import->status === \App\Models\SpmImport::STATUS_COMMITTED)
    ...
    @if ($import->status === \App\Models\SpmImport::STATUS_STAGED && ! $import->kedaluwarsa())
        <div class="nav" style="margin-top:8px;">
            <form method="POST" action="{{ route('manajemen-data.import.spm.batalkan', $import) }}" onsubmit="return confirm('Batalkan pemeriksaan berkas ini? Berkas perlu diunggah ulang bila ingin dilanjutkan.');">
                @csrf
                @method('DELETE')
                <button type="submit" class="btn">Batalkan</button>
            </form>
            <form method="POST" action="{{ route('manajemen-data.import.spm.konfirmasi', $import) }}" onsubmit="return confirm('Simpan {{ $import->jumlah_baru + $import->jumlah_update }} baris (baru + update) ke SPM {{ $labelJenis }}? Baris yang ditolak tidak akan disimpan.');">
                @csrf
            ...
            <tbody>
                @forelse ($baris as $b)
                    <tr>
                        ...
                        <td>{{ $b->nomor_dokumen ?? '—' }}</td>
                        <td>{{ $b->tanggal_dokumen?->format('d-m-Y') ?? '—' }}</td>
                        @if ($import->jenis_spm === 'ls')
                            <td>{{ $b->kode_rekening ?? '—' }}{{ $b->tagging_nama ? ' ('.$b->tagging_nama.')' : '' }}</td>
                        @endif
                        <td>{{ $b->nominal !== null ? 'Rp '.fmt_rupiah($b->nominal) : '—' }}</td>
                        @if ($import->jenis_spm === 'ls')
                            <td>{{ $b->sisa_sebelum !== null ? 'Rp '.fmt_rupiah($b->sisa_sebelum) : '—' }}</td>
                            <td style="{{ $b->sisa_sesudah !== null && $b->sisa_sesudah < 0 ? 'color:var(--err-teks);font-weight:700;' : '' }}">{{ $b->sisa_sesudah !== null ? 'Rp '.fmt_rupiah($b->sisa_sesudah) : '—' }}</td>
                            <td>
                                {{ $b->penerima ?? '—' }}
