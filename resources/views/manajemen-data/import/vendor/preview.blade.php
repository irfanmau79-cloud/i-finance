...
<div class="dash-card">
    ...
    @if ($import->status === \App\Models\VendorImport::STATUS_COMMITTED)
    ...
    @if ($import->status === \App\Models\VendorImport::STATUS_STAGED && ! $import->kedaluwarsa())
        <div class="nav" style="margin-top:8px;">
            <form method="POST" action="{{ route('manajemen-data.import.vendor.batalkan', $import) }}" onsubmit="return confirm('Batalkan pemeriksaan berkas ini? Berkas perlu diunggah ulang bila ingin dilanjutkan.');">
                @csrf
                @method('DELETE')
