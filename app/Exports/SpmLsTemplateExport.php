...
class SpmLsTemplateExport implements FromArray, PunyaPetunjukKolom, ShouldAutoSize, WithEvents
{
    ...
    /** 19 kolom -> kolom terakhir S. */
    private const KOLOM_TERAKHIR = 'S';

    /** Kolom tanggal. */
    private const KOLOM_TANGGAL = ['A', 'C'];

    /**
     * Kolom yang harus dibaca sebagai TEKS. Kode rekening supaya
     * "5.1.02.01.01.0024" tidak jadi angka/tanggal, dan Nomor Rekening supaya
     * nol di depannya ("0011223344") tidak hilang.
     */
    private const KOLOM_KODE = ['E', 'G', 'R'];
    ...
    public const CATATAN = 'Satu baris = satu MATA ANGGARAN dalam sebuah dokumen SPM LS. Satu dokumen boleh mencakup beberapa mata anggaran: tulis satu baris per mata anggaran, dan ulangi Tanggal SPM, Nomor SPM, SP2D, PPN, PPh, Penerima, Bank Tujuan, Nomor Rekening, serta Uraian SAMA PERSIS di semua baris milik dokumen itu - kalau berbeda, seluruh dokumen ditolak. Yang WAJIB diisi: Tanggal SPM, Nomor SPM, mata anggaran (Kode Sub Kegiatan, Sub Kegiatan, Kode Rekening, dan Tagging bila mata anggarannya bertagging), Nominal bruto, Penerima, serta Uraian. Satu dokumen disimpan sekaligus atau tidak sama sekali: bila satu barisnya ditolak, baris lain pada dokumen yang sama ikut dibatalkan. Berbeda dengan UP/GU/TU, SPM LS MENGURANGI pagu dan masuk sebagai realisasi, sehingga total per mata anggaran tidak boleh melebihi sisa tersedia.';
    ...
    public const PETUNJUK = [
        ...
        ['Tagging', 'Bersyarat', 'Teks', 'Tagging mata anggaran. WAJIB diisi bila mata anggaran itu memang bertagging - tanpa itu barisnya ditolak karena mata anggarannya tidak bisa ditentukan secara pasti. Kosongkan HANYA bila mata anggarannya memang tanpa tagging. Tagging yang belum terdaftar akan ditolak, bukan dibuat baru.', 'Rutin'],
